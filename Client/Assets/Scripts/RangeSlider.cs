    ...
    public class RangeSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        ...
        public float MinRangeSize
        {
            get
            {
                if (WholeNumbers)
                {
                    return Mathf.Round(m_MinRangeSize);
                }
                return m_MinRangeSize;
            }
            set
            {
                if (!m_MinRangeSize.Equals(value))
                {
                    m_MinRangeSize = Mathf.Clamp(value, 0f, MaxValue - MinValue);
                    SetLow(m_LowValue);
                    SetHigh(m_HighValue);
                    UpdateVisuals();
                }
        ...
        protected virtual void SetLow(float input, bool sendCallback)
        {
            ...
            // Clamp the input
            float newValue = Mathf.Clamp(input, MinValue, HighValue - MinRangeSize); //clamp between min and High
            if (WholeNumbers)
            {
        ...
        {
            // Clamp the input
            float newValue = Mathf.Clamp(input, LowValue + MinRangeSize, MaxValue); //clamp between min and High
            if (WholeNumbers)
            {
                newValue = Mathf.Round(newValue);
            }
